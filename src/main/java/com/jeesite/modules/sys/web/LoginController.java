...
public class LoginController extends BaseController{
	...
	public String login(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		// 是否显示验证码
		model.addAttribute("isValidCodeLogin", "200".equals(Global.getConfig("sys.login.failedNumAfterValidCode")));
	...
	public String index(HttpServletRequest request, HttpServletResponse response, Model model) {
		// 获取当前登录用户，判断用户所在的组织机构使用系统的时间段
		User curUser = UserUtils.getUser();
		if(curUser != null && !curUser.isSuperAdmin()) {
			Employee employee = (Employee)curUser.getRefObj();
			if(employee != null) {
				String startTime = employee.getOffice().getExtend().getExtendS1();
				String endTime = employee.getOffice().getExtend().getExtendS2();
				SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd");
				SimpleDateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				String startDateTimeS = df1.format(new Date())+" "+startTime+":00";
				String endDateTimeS = df1.format(new Date())+" "+endTime+":00";
				try {
					Date startDateTimeD = df2.parse(startDateTimeS);
					Date endDateTimeD = df2.parse(endDateTimeS);
					Date curDateTime = new Date();
			        if(curDateTime.after(startDateTimeD) && curDateTime.before(endDateTimeD)) {
			        	System.out.println("当前时间用户可以登录使用系统.");
			        } else {
			        	// 当前时间用户不能使用系统
			        	model.addAttribute("message", text("当前时间不允许使用"));		        			        
			        	return REDIRECT + adminPath + "/notArrowUse";
			        }
		...
		if (!"true".equals(Global.getConfig("shiro.isAllowRefreshIndex"))){
	...
	@RequestMapping(value = "notUseLogout")
	public String notUserLogout(HttpServletRequest request, HttpServletResponse response, Model model) {
		UserUtils.getSubject().logout();
		return REDIRECT + adminPath + "/login";
	}
	 ...
	 */
	@RequiresPermissions("user")
	@RequestMapping(value = "notArrowUse")
	public String notArrowUse(HttpServletRequest request, HttpServletResponse response, Model model) {
		return "modules/sys/notArrowUse";
	}
